...
class TFC(nn.Module):
    def __init__(self, c_in, c_out, l, k, bn_norm):
        ...
        for i in range(l):
            current_c_in = c_in if i == 0 else c_out
            self.H.append(
                nn.Sequential(
                    get_norm(bn_norm, current_c_in),
                    nn.ReLU(),
                    nn.Conv2d(in_channels=current_c_in, out_channels=c_out, kernel_size=k, stride=1, padding=k // 2),
                )
            )
